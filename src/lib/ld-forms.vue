    ...
    <div v-else class="form wh">
      ...
      <div v-if="forms&&Object.keys(forms).length>0" class="el-form w h"
        ...
        <div class="a-i-c w" :class="{'f-b-w':!isRow,'f-s-w':isRow}">
          <div v-if="item['visabled']!=false" v-for="(item,i) in layouts" class="el-form-item position-relative"
            :key='i'
            ...
            <div class="w" :class="{'el-disabled':getDisabled(item),'el-readonly':getDisabled(item)}">
              <div v-if="layoutType.includes(item['type'].toLocaleLowerCase())" class="a-i-c w"
                ...
                <div v-if="item['label']||item['showLabel']" class="el-form-item__label f-n-c-w a-i-c f-r"
                  ...
                  :style="{'width':isRow?'auto':labelWidth,'padding-left':isRow?'16px':'auto'}" style="flex-shrink: 0;">
                ...
                <div v-else class=" box-b over-h" style="flex-grow: 2;"
                  :style="{'cursor':getDisabled(item)?'no-drop':'default','flex-grow':isRow?'1':'2'}"
                  :class="{'el-input--suffix':showInputSuffix(item)}">
                  <!-- ================ 组件开始 start ================ -->
                  ...
                  <template v-if="isControlsType(item,'date')">
                    <el-date-picker v-if="item['dateType']=='week'" class="w" style="width: 100%;"
                      :class="getStyleOrCss({'error-bor-d':item['error']==true},item,'css')" :style="item['style']"
                    ...
                    <el-date-picker v-else-if="item['dateType'].indexOf('range')>0" class="w" style="width: 100%;"
                      ...
                      :start-placeholder="item['startPlaceholder']||`开始${rangePlaceholder[item['dateType']]}`"
  ...
  export default {
    ...
    data() {
      return {
         ...
         * 日期范围提示文字
         */
        rangePlaceholder: config.rangePlaceholder,
...
<style>
  ...
  .el-date-editor.el-range-editor.el-input__inner.w.el-date-editor--monthrange,
  .el-date-editor.el-range-editor.el-input__inner.w.el-date-editor--daterange,
  .el-date-editor.el-range-editor.el-input__inner.w.el-date-editor--datetimerange {
    display: flex !important;
    justify-content: flex-start !important;
    padding-right: 0 !important;
  }

  .el-date-editor.el-range-editor.el-input__inner.w.el-date-editor--monthrange>.el-range-input,
